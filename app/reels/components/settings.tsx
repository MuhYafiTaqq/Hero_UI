...
export default function Settings({ images, setImages }: { images: string, setImages: (images: string) => void }) {
  ...
  const downloadImage = async () => {
        try {
      const { error } = await supabase.from("crop_reels").insert({});
      ...
      <div className="md:col-span-13 md:p-6 p-2 rounded-xl bg-custom-3 row-span-13 md:row-span-1">
        ...
        >
          ...
          >
            <canvas
              ref={canvasRef}
              className="border border-black dark:border-white rounded block"
              style={{
                maxWidth: "100%",
          ...
          <div>
            ...
            >
              <div className="overflow-hidden flex flex-col gap-3 origin-top">
                <div className="space-y-1">
                  <label className="flex items-center justify-between md:text-md text-sm font-medium text-black dark:text-white">
            ...
            >
              <div className="overflow-hidden flex flex-col gap-2 origin-top">
                <span className="tracking-wide text-md font-medium text-black dark:text-white">
                  Custom Background
                </span>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-9 w-9 p-0 border border-black/20 dark:border-white/50 rounded cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 transition"
                  />
                  <input
                    disabled={mode !== "custom"}
                    value={customInput}
                    ...
                    className="flex-1 h-9 text-xs px-4 rounded-full border border-black/25 outline-none focus:ring-2 ring-blue-400/50 bg-white dark:bg-white/40 disabled:opacity-50 disabled:cursor-not-allowed transition"
                  />
                  <button
                    type="button"
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 md:py-3 py-2 md:text-md text-sm font-medium border border-black/50 dark:border-white/50 w-full rounded-full transition duration-500 ${
                  !imageRef.current || exporting
                    ? "bg-gray-300 text-gray-600 cursor-not-allowed"
                    : "text-black hover:bg-black/10 active:scale-[.97] dark:text-white dark:hover:bg-white/10"
                }`}
              >
                Back
              </button>
              <button
                ...
                className={`px-4 md:py-3 py-2 md:text-md text-sm font-medium border w-full rounded-full transition duration-500 ${
                  !imageRef.current || exporting
                    ? "bg-gray-300 text-gray-600 cursor-not-allowed"
                    : "bg-black text-white hover:bg-green-500 active:scale-[.97] dark:bg-white dark:text-black"
                }`}
              >
...
function ModeSwitch({
  ...
  return (
    <div className="relative flex md:h-10 h-8 w-full rounded-full bg-custom-3  select-none overflow-hidden shadow-sm md:border-4 border-2 border-custom-3 mb-5">
      <div
        // Indicator hitam
        className="absolute top-0 bottom-0 left-0 rounded-full bg-black dark:bg-white shadow-md transition-transform duration-500 ease-in-out will-change-transform"
        style={{
        ...
        return (
          <button
            ...
            className={`relative z-10 flex-1 md:text-md text-sm font-bold first-letter:uppercase tracking-wide transition-colors duration-200 cursor-pointer ${
              active ? "text-white dark:text-black" : "text-black/60 hover:text-black dark:text-white/60 hover:dark:text-white"
            }`}
          >
            {m}
          </button>
